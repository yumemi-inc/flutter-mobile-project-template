...
3. Update the versions of the `sdk` and `flutter` parts of each package in `pubspec.yaml`
...
> [!NOTE]
> The rule version of yumemi_lints needs to be updated in `analysis_options.yaml`, which is updated automatically by the post hooks in `dart pub get`.
>
>```yaml
