...
if [ -n "$latest_base" ]; then
  if [ "$latest_base" != "$default_branch" ] && git show-ref --verify --quiet refs/heads/$default_branch; then
    # 派生元ブランチの最新の共通コミットが 基準ブランチ に取り込まれているか確認
    if git branch --contains $latest_commit | grep -q "$default_branch"; then
...
# 変更されたファイル一覧を表示
echo -e "\n変更されたファイル一覧:"
git diff --name-status $latest_base...$current_branch | cat

# 差分の表示
echo -e "\nブランチ $latest_base と $current_branch の差分:"
git diff $latest_base...$current_branch | cat
